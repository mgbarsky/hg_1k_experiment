...
# we have a selection of keys in file FILE_NAMES_...txt
# we have class labels for these samples in FILE_LABELS_....csv
# we are going to read each line
# select only columns of interest (according to file_names (or file_labels...))
# choose only lines that are listed in lines file
# convert each line into ped format
...
GetOptions("snips=s" => \$snipTableFile,
		...
		"lines=s" => \$linesFileName);

#collect program arguments
if($snipTableFile eq "" or  $originalColumnsFile eq "" or $sampleKeyLabelsFile eq "" or $linesFileName eq "")
{
    print "Error: a table of snips file, the column names file, and the selected sample-labels files  must be provided: --snips FILE NAME --columns FILE NAME --labels FILE NAME --lines FILE NAME\n";   
    exit 1;
}

#open output file for writing - ped encoding
open WRITER, '>', $snipTableFile."SELECTEDLINES.ped" or die "error trying to open output file: $!";
...
#read line numbers and put them into a dictionary lineNumbersDict
 my $totalSelectedLines = 0;
open my $reader, $linesFileName or die "error trying to open dictionary file for reading selected lines: $!";
while (my $line = <$reader>)
{
   	chomp $line;
	my $numkey = int ($line);
#print $key.' '.$val;
	$lineNumbersDict{$numkey}=$line;
	$totalSelectedLines++;
}
close $reader;
...
print "Processing file $snipTableFile with $totalSamples samples from which I want to select only $totalSelectedSamples samples and $totalSelectedLines markers.\n";
...
while(my $line = <$reader>)
{
	...
	#we check if this is a line of interest, and process it if yes
	if (exists $lineNumbersDict{$lineID})
	{
		$selectedSnipsCount++;
		my $pedLineIndex =0;
